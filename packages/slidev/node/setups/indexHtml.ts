...
export default async function setupIndexHtml({ mode, entry, clientRoot, userRoot, roots, data, base }: Omit<ResolvedSlidevOptions, 'utils'>): Promise<string> {
  ...
  // Auto-generate OG image path if enabled and no custom image is provided
  const ogImageToUse = seoMeta.ogImage || (data.headmatter.generateOgImage ? './og.png' : undefined)
    ...
    init: [
      {
        ...
        meta: [
          ...
          { property: 'og:image', content: ogImageToUse },
